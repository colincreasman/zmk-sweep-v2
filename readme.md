# sweep zmk-config (urob's config, QWERTY base)

This is a near-verbatim copy of [urob's ZMK
config](https://github.com/urob/zmk-config) adapted for the **Ferris Sweep**
(`nice_nano_v2` + `cradio` shield). The only change to the layout is the
...
magic-shift, nav cluster, mouse emulation, etc.) is identical to upstream.
